-- Query 1: Customers who have not paid their bills
SELECT c.FullName, b.Month, b.TotalAmount, b.PaymentStatus
FROM Customer c
JOIN Billing b ON c.CustomerID = b.CustomerID
WHERE b.PaymentStatus = 'Unpaid';

-- Query 2: Subscriber count for each payment plan
SELECT p.PlanName, COUNT(s.SubscriptionID) AS TotalSubscribers
FROM Plan p
JOIN Subscription s ON p.PlanID = s.PlanID
GROUP BY p.PlanName;

-- Query 3: Top data-using customer
SELECT c.FullName, SUM(u.DataUsedMB) AS TotalDataMB
FROM Customer c
...
SELECT s.SubscriptionID, c.FullName, p.PlanName, s.Status
FROM Subscription s
JOIN Customer c ON s.CustomerID = c.CustomerID
JOIN Plan p ON s.PlanID = p.PlanID
WHERE s.Status = 'Active';

-- Query 5: Support tickets and assigned employees
...
-- Query 7: Latest billing info for each customer
